/**************************************
@Author: Sliverkiss (修改适配小程序)
@Date: 2023-08-06 19:20:18
@Description: 
亚朵酒店小程序 签到、抽奖

使用教程：
 1. 在 Loon/QX/Surge 中配置重写规则
 2. 打开亚朵小程序，进行一次签到操作
 3. 若提示"获取签到Cookie成功"，则可关闭重写
 4. 每日自动运行签到和抽奖任务

【Loon】:
[Script]
cron "0 30 7 * * *" script-path=adjd.js, timeout=300, tag=亚朵酒店小程序
http-request ^https:\/\/miniapp\.yaduo\.com\/atourlife\/signIn\/signIn script-path=adjd.js, timeout=10, tag=亚朵酒店获取Cookie
...
【Quantumult X】:
[rewrite_local]
^https:\/\/miniapp\.yaduo\.com\/atourlife\/signIn\/signIn url script-request-header adjd.js

[task_local]
0 30 7 * * * adjd.js, tag=亚朵酒店小程序, enabled=true
...
function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        // 提取 Cookie 和 Passtoken
        const cookie = headers['Cookie'] || headers['cookie'];
        const passtoken = headers['Passtoken'] || headers['passtoken'];
        ...
        const signHeader = {
            ...
            'User-Agent': headers['User-Agent'] || 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6_1 like Mac OS X) AppleWebKit/605.1.15',
        ...
        if (signHeader && cookie && passtoken) {
            $.setdata(JSON.stringify(signHeader), 'adjd_header');
            $.setdata(signUrlVal, 'adjd_url');
            $.msg($.name, "", `✅ 获取Cookie成功\nCookie: ${cookie.substring(0, 30)}...\nPasstoken: ${passtoken.substring(0, 30)}...`);
        } else {
            $.msg($.name, "", "❌ 获取Cookie失败\n缺少必要字段，请在小程序内重新操作");
        }
    }
